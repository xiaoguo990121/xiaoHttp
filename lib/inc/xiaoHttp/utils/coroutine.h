...
namespace xiaoHttp
{
    ...
    template <typename T = void>
    struct CallbackAwaiter : public xiaoNet::NonCopyable
    {
        bool await_ready() noexcept
        {
            return false;
    ...
    template <>
    struct CallbackAwaiter<void> : public xiaoNet::NonCopyable
    {
        bool await_ready() noexcept
        {
            return false;
    ...
    namespace internal
    {
        ...
        {
            TimerAwaiter(xiaoNet::EventLoop *loop,
                         const std::chrono::duration<double> &delay)
                : loop_(loop), delay_(delay.count())
            {
            }

            TimerAwaiter(xiaoNet::EventLoop *loop, double delay)
        ...
        {
            LoopAwaiter(xiaoNet::EventLoop *workLoop,
                        std::function<void()> &&taskFunc,
                        xiaoNet::EventLoop *resumeLoop = nullptr)
                : workLoop_(workLoop),
        ...
        private:
            xiaoNet::EventLoop *workLoop_{nullptr};
            xiaoNet::EventLoop *resumeLoop_{nullptr};
            std::function<void()> taskFunc_;
        };

        struct [[nodiscard]] SwitchThreadAwaiter : CallbackAwaiter<void>
        {
            explicit SwitchThreadAwaiter(xiaoNet::EventLoop *loop) : loop_(loop)
            {
            }

            void await_suspend(std::coroutine_handle<> handle)
            {
                loop_->runInLoop([handle]()
                                 { handle.resume(); });
            }
        ...
        {
            EndAwaiter(xiaoNet::EventLoop *loop) : loop_(loop)
            {
                assert(loop);
            }
            ...
            void await_suspend(std::coroutine_handle<> handle)
            {
                loop_->runOnQuit([handle]()
                                 { handle.resume(); });
            }

        private:
            xiaoNet::EventLoop *loop_{nullptr};
        };
    ...
    inline internal::TimerAwaiter sleepCoro(xiaoNet::EventLoop *loop,
    ...
    inline internal::SwitchThreadAwaiter switchThreadCoro(
        xiaoNet::EventLoop *loop) noexcept
    {
        assert(loop);
        return internal::SwitchThreadAwaiter{loop};
    }

    inline internal::EndAwaiter untilQuit(xiaoNet::EventLoop *loop)
    {
        assert(loop);
        return {loop};
    }
    ...
    {
        template <typename T>
        struct [[nodiscard]] EventLoopAwaiter : public xiaoHttp::CallbackAwaiter<T>
        {
            EventLoopAwaiter(std::function<T()> &&task, xiaoNet::EventLoop *loop)
     ...
     */
    template <typename T>
    inline internal::EventLoopAwaiter<T> queueInLoopCoro(xiaoNet::EventLoop *loop,
                                                         std::function<T()> task)
    {
