...
namespace xiaoHttp
{
    ...
    public:
        ...
        HttpAppFramework &registerNewConnectionAdvice(
            const std::function<bool(const xiaoNet::InetAddress &,
                                     const xiaoNet::InetAddress &)> &advice)
            override;
        ...
        HttpAppFramework &registerSessionDestroyAdvice(
            const AdviceDestorySessionCallback &advice) override
        {
        ...
        HttpAppFramework &setLogLevel(xiaoLog::Logger::LogLevel level) override;
        ...
        xiaoNet::EventLoop *getLoop() const override;

        xiaoNet::EventLoop *getIOLoop(size_t id) const override;
        ...
        std::vector<xiaoNet::InetAddress> getListeners() const override;
        ...
        {
            return xiaoNet::utils::tlsBackend() != "None";
        }

        size_t getCurrentThreadIndex() const override
        {
            auto *loop = xiaoNet::EventLoop::getEventLoopOfCurrentThread();
            if (loop)
            {
                return loop->index();
            }
    ...
    private:
        ...
        std::string serverHeader_{"server: drogon/" + xiaoHttp::getVersion() +
                                  "\r\n"};
        ...
        size_t threadNum_{1};
        std::unique_ptr<xiaoNet::EventLoopThreadPool> ioLoopThreadPool_;
        ...
        std::vector<AdviceDestorySessionCallback> sessionDestroyAdvices_;
        SessionManager::IdGeneratorCallback sessionIdGeneratorCallback_;
        std::shared_ptr<xiaoLog::AsyncFileLogger> asyncFileLoggerPtr_;
