...
void HttpRequestImpl::reserveBodySize(size_t length)
{
    ...
    if (length <= HttpAppFrameworkImpl::instace().getClientMaxMemoryBodySize())
    {
...
void HttpRequestImpl::appendToBuffer(xiaoNet::MsgBuffer *output) const
{
    switch (method_)
    {
    case Get:
...
StreamDecompressStatus HttpRequestImpl::decompressBody()
{
    ...
    if (contentEncoding.empty() || contentEncoding == "identity")
    {
        ...
        retrun StreamDecompressStatus::Ok;
    }
#ifdef USE_BROTLI
#endif
...
void HttpRequestImpl::startFinish()
{
    assert(loop_->isInLoopThread());
    assert(streamStatus_ == ReqStreamStatus::Open);
