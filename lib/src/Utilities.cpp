...
namespace xiaoHttp
{
    namespace utils
    {
        ...
        xiaoLog::Date getHttpDate(const std::string &httpFullDateString)
        {
            ...
            for (const char *format : formats)
            {
                if (strptime(httpFullDateString.c_str(), format, &tmptm) != NULL)
                {
                    ...
                    return xiaoLog::Date(epoch * MICRO_SECONDS_PRE_SEC);
                }
            }
            LOG_WARN << "invalid datetime format: '" << httpFullDateString << "'";
            return xiaoLog::Date((std::numeric_limits<int64_t>::max)());
        }
